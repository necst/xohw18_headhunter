...
void convShiftData(hls::stream<data_t> &streamIn, hls::LineBuffer<KH, FM_IN*DIMW, data_t> &lineBuf){
	line_shift: for (int i = 0; i < FM_IN*DIMW; i++){
		...
		data_t val = (i < KP*FM_IN || i >= (DIMW*FM_IN)-(KP*FM_IN) )
				? 0
				: streamIn.read();

		//if(i < KP*FM_IN || i >= (DIMW*FM_IN)-(KP*FM_IN) )
		//	std::cout << "Inject pad on index shift Line Buffer " << i << "\n";
...
void convLayer (hls::stream<data_in> &streamIn, hls::stream<data_out> &streamOut, data_in weights[FM_OUT][FM_IN][KH][KW], data_in bias[FM_OUT], hls::stream<my_ap_axis<ap_uint<32>,32,1,1,1> > &axiStreamOut){
    ...
    //static hls::Window<KH, KW, data_in> window;
    ...
    line_load: for(int i = 0; i < FM_IN*KH*DIMW; i++){
		#pragma HLS PIPELINE II=1
    	data_in val = ( i < (KP*DIMW*FM_IN) //top padding
    			|| ((i/FM_IN)%DIMW) < KP //left padding
				|| ((i/FM_IN)%DIMW) >= DIMW-KP ) //right padding
				? 0
            	: streamIn.read();

    	/*if ( i < (KP*DIMW*FM_IN) )//top padding
                std::cout << "Inject pad on top index i = " << ((i/FM_IN)/DIMW) << " j = "  << ((i/FM_IN)%DIMW) << " fm = " << i%FM_IN << "\n";

            else if ( ((i/FM_IN)%DIMW) < KP )//left padding
                std::cout << "Inject pad on left index i = " << ((i/FM_IN)/DIMW) << " j = "  << ((i/FM_IN)%DIMW) << " fm = " << i%FM_IN << "\n";

            else if ( ((i/FM_IN)%DIMW) >= DIMW-KP )//right padding
                std::cout << "Inject pad on right index i = " << ((i/FM_IN)/DIMW) << " j = "  << ((i/FM_IN)%DIMW) << " fm = " << i%FM_IN << "\n";
    	 */
    ...
    fm_height: for (int i = 0; i < DIMH-KH+1; i+=KS) {
    	fm_width: for (int j = 0; j < DIMW-KW+1; j+=KS) {
    		...
    		fm_out: for (int k = 0; k < FM_OUT; k++) {
    			...
    			fm_in: for (int l = 0; l < FM_IN; l++) {
					#pragma HLS PIPELINE II=1//#pragma HLS UNROLL

    				/*window_load: for(int w = 0; w < KH*KW; w++) {
                    		int kh = w / (KW);
                    		int kw = w % (KW);
                    		window.insert(lineBuf.getval( kh, kw + j*FM_IN + l), kh, kw);
                    	}
                    	std::cout << "******\n";*/
    				...
    				ker_height: for (int s = 0; s < KH; s++) {
    					...
    					ker_width: for (int t = 0; t < KW; t++) {
    						...
    						data_out pixel = lineBuf.getval(s, ((j+t)*FM_IN) + l ); //window.getval(s, (t*FM_IN) + l);
    	...
    	if (i+KS-1+KH < DIMH-KP) {
    		new_line_load:for(int ks = 0; ks < KS; ks++) {
    			convShiftData<FM_IN, KH, KP, DIMH, DIMW, data_in> (streamIn, lineBuf);
    		}
    	} else if (i+KH < DIMH-KP) { // data with padding injection

    		new_line_load_final:for(int ks = 0; ks < (DIMH-KP)%KH; ks++){
    			convShiftData<FM_IN, KH, KP, DIMH, DIMW, data_in> (streamIn, lineBuf);
    		}

    		final_pad_miss:for(int kp = 0; kp < KS - ((DIMH-KP)%KH); kp++){

    			std::cout << "Padding injection\n";
    			line_shift_final_miss: for (int i = 0; i < FM_IN*DIMW; i++){
					#pragma HLS PIPELINE II=1

    				lineBuf.shift_up(i);
    				lineBuf.insert_top(0, i);
    			}
    		}
    	} else { // padding injection
    		final_pad:for(int kp = 0; kp < KS; kp++){

    			std::cout << "Final padding injection\n";
    			line_shift_final: for (int i = 0; i < FM_IN*DIMW; i++){
					#pragma HLS PIPELINE II=1
