...
void convAll(hls::stream<my_ap_axis<ap_uint<DATA_WIDTH>,DATA_WIDTH,1,1,1> > &axiStreamIn,
...
int main()
{
	...
	convAll(axiStreamIn, axiStreamOut, CONV_WEIGHT);
	convAll(axiStreamIn, axiStreamOut, COMPUTE);
	...
	for(int s = 0; s < SAMPLES; s++){
		...
		conv_layer_cpu <FM_0, FM_1, K_1> (DIM_0, KS_1, img[s], res_cpu[s], w1_conv, b1_conv, 0);
